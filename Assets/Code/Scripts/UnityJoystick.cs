...
public class UnityJoystick : MonoBehaviour
{
    ...
    */
    ...
    private static List<UnityJoystick> joysticks;
    ...
    private void Awake()
    {
        ...
        if (!enumeratedJoysticks)
        {
            try
            {
                ...
                joysticks = new List<UnityJoystick>(objs.Length);
                foreach (GameObject obj in objs)
                {
                    UnityJoystick newJoystick = obj.GetComponent<UnityJoystick>();
                    if (newJoystick == null)
                    {
        ...
        else
        {
            for (int i = 0; i < count; i++)
            {
                ...
                if (shouldLatchFinger && (lastFingerId == -1 || lastFingerId != touch.fingerId))
                {
                    ...
                    foreach (UnityJoystick j in joysticks)
                    {
                        if (j == this)
                        {
                            continue;
