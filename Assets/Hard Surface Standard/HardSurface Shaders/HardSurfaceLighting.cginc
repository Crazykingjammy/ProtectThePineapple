...
inline fixed4 LightingBlinnPhongHardsurfaceFront (SurfaceOutput s, fixed3 lightDir, fixed3 viewDir, fixed atten)
{
	...
	fixed4 c;
		...
		// Alpha test wont handle overbright color ranges saturate keeps it in range
		c.rgb = saturate ((s.Albedo * _LightColor0.rgb * diff + _LightColor0.rgb * _SpecColor.rgb * spec) * (atten * 2));
	#else
